...
profilePage: async (req, res, next) => {
  const userId = req.userId;
  const books = await borrowerService.getAllBorrowedBooks(userId, next);

  const booksWithFine = await Promise.all(
    books.map(async (entry) => {
      let fineAmount = 0;
      if (!entry.active) {
        const fine = await fineService.getUserFineByBook(userId, entry.bookId._id);
